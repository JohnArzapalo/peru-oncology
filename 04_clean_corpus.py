...
Qué hace  : (1) reglas deterministas recortan front-matter/resumen-EN/referencias y,
            además, normalizan y quitan material no clínico inline (citas, figuras, URLs);
            (2) el LLM solo MARCA puntos de corte y el código recorta por slicing (nunca
            reescribe). Por (1) el texto limpio NO es copia carácter-a-carácter del crudo;
            los offsets de las anotaciones son exactos sobre el texto publicado.
...
documento (que hizo cada etapa).
  ...
  Precisión: el LLM solo MARCA puntos de corte y el código recorta por slicing (nunca
  reescribe). Las reglas deterministas, en cambio, normalizan el texto y eliminan
  material no clínico inline (citas, llamadas a figuras, URLs), por lo que el texto limpio
  NO es copia carácter-a-carácter del crudo. Los offsets de las anotaciones son exactos
  sobre el texto publicado (verificado: 17 657 spans, 0 mismatches).
...
def main():
    ...
    print("\n  El LLM solo marca puntos de corte y el codigo recorta (nunca reescribe);")
    print("  las reglas ademas normalizan y quitan material no clinico (citas, figuras, URLs).")
