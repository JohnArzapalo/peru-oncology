...
```
   ...
   03_filter_candidates     ──►  seleccionados/        (descarta veterinaria / inglés / sin estructura)
                  │
   04_clean_corpus          ──►  textos_limpios_final/ (reglas deterministas + DeepSeek; el LLM solo marca cortes, el código recorta)
                  │
...
```bash
git clone https://github.com/JohnArzapalo/peru-oncology.git
cd peru-oncology
python -m venv .venv
...
defecto (esa carpeta está en `.gitignore`). Los valores de parámetros que se muestran abajo
(p. ej. `--n`, `--from-year`, `--until-year`) son **ilustrativos**; los parámetros y las
cifras exactas del estudio (incluidos los 170 documentos finales) se reportan en el paper.
...
- Las etapas con LLM son **asistencia, no decisión final**: en la limpieza, el LLM solo
  marca puntos de corte y el código recorta (nunca reescribe), mientras que las reglas
  deterministas normalizan el texto y eliminan material no clínico (citas, llamadas a
  figuras, URLs); los offsets de las anotaciones son exactos sobre el texto publicado
  (verificado: 17 657 spans, 0 mismatches). La anotación es un **borrador**: el *gold
  standard* lo revisa y corrige un anotador humano con apoyo clínico.
